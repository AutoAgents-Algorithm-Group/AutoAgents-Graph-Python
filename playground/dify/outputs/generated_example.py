...
def main():
    ...
    workflow.add_node(
        id="section_iterator",
        position={'x': 1209.9734511109368, 'y': 486.7408933479527},
        state=DifyIterationState(
            title="章节迭代器",
            error_handle_mode="terminated",
            input_parameters=[{'value_selector': ['outline_generator', 'text'], 'variable': 'outline'}],
            is_array_input=True,
            is_parallel=False,
            iterator_selector=['1749449203563', 'result'],
            output_selector=['1749443261566', 'text'],
    ...
    workflow.add_node(
        ...
        position={'x': 24, 'y': 68},
        state=DifyIterationStartState(
            title="",
            isInIteration=True,
            desc="",
        ),
        parent_id="section_iterator"
    )
...
def main(**kwargs) -> dict:
  ...
  return {
    ...
    workflow.add_edge("section_iteratorstart0", "1749443261566")
    workflow.add_edge("outline_generator", "1749449203563")
    workflow.add_edge("1749449203563", "section_iterator")
